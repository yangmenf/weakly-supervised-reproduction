...
def build_validation(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            # Fix: labels.shape[1:] might include channel dimension, use only spatial dimensions
            ...
            resized_sm = F.interpolate(score_maps, size=target_size, mode='bilinear', align_corners=False)
            ...
            resized_segs = F.interpolate(segs, size=target_size, mode='bilinear', align_corners=False)
