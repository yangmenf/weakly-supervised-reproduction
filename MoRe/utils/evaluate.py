import numpy as np
import sklearn.metrics as metrics

def multilabel_score(y_true, y_pred):

    return metrics.f1_score(y_true, y_pred)


def _fast_hist(label_true, label_pred, num_classes):
    mask = (label_true >= 0) & (label_true < num_classes)
    hist = np.bincount(
        num_classes * label_true[mask].astype(int) + label_pred[mask],
        minlength=num_classes ** 2,
    )
    return hist.reshape(num_classes, num_classes)

def scores(label_trues, label_preds, num_classes=21):
    hist = np.zeros((num_classes, num_classes))
    for lt, lp in zip(label_trues, label_preds):
        lt_f = lt.flatten()
        lp_f = lp.flatten()
        # Defensive check: ensure the two arrays are aligned in length.
        # If shapes differ (e.g., occasional resize mismatch), truncate to the min length
        # to avoid boolean mask shape errors while still contributing usable pixels.
        if lt_f.size != lp_f.size:
            min_len = min(lt_f.size, lp_f.size)
            lt_f = lt_f[:min_len]
            lp_f = lp_f[:min_len]
        hist += _fast_hist(lt_f, lp_f, num_classes)
    acc = np.diag(hist).sum() / hist.sum()
    _acc_cls = np.diag(hist) / hist.sum(axis=1)
    acc_cls = np.nanmean(_acc_cls)
    iu = np.diag(hist) / (hist.sum(axis=1) + hist.sum(axis=0) - np.diag(hist))
    valid = hist.sum(axis=1) > 0  # added
    mean_iu = np.nanmean(iu[valid])
    freq = hist.sum(axis=1) / hist.sum()
    cls_iu = dict(zip(range(num_classes), iu))

    TP = np.diag(hist)
    FN = hist.sum(axis=1) - TP ## hist.sum(axis=1) : label 里的阳
    FP = hist.sum(axis=0) - TP ## hist.sum(axis=0)   模型预测的阳

    cr = FP / TP
    cr = dict(zip(range(num_classes), cr))

    precision = TP / (TP + FP)
    precision = dict(zip(range(num_classes), precision))
    recall = TP / (TP + FN)
    recall = dict(zip(range(num_classes), recall))

    return {
        "pAcc": acc,
        "mAcc": acc_cls,
        "miou": mean_iu,
        "iou": cls_iu,
        "confusion": cr,
        "precision":precision,
        "recall": recall 
    }

def pseudo_scores(label_trues, label_preds, num_classes=21):
    hist = np.zeros((num_classes, num_classes))
    for lt, lp in zip(label_trues, label_preds):
        lt = lt.flatten()
        lp = lp.flatten()
        lt[lp==255] = 255
        lp[lp==255] = 0
        hist += _fast_hist(lt, lp, num_classes)
    acc = np.diag(hist).sum() / hist.sum()
    acc_cls = np.diag(hist) / hist.sum(axis=1)
    acc_cls = np.nanmean(acc_cls)
    iu = np.diag(hist) / (hist.sum(axis=1) + hist.sum(axis=0) - np.diag(hist))
    valid = hist.sum(axis=1) > 0  # added
    mean_iu = np.nanmean(iu[valid])
    freq = hist.sum(axis=1) / hist.sum()
    cls_iu = dict(zip(range(num_classes), iu))

    return {
        "pAcc": acc,
        "mAcc": acc_cls,
        "miou": mean_iu,
        "iou": cls_iu,
    }