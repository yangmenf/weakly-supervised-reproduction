...
parser.add_argument("--work_dir", default="/root/autodl-tmp/experiments/MoRe/outputs", type=str, help="w_outputs")
parser.add_argument("--log_tag", default="train_voc", type=str, help="train_voc")

### dataset utils
parser.add_argument("--data_folder", default='/root/autodl-tmp/data/VOC/VOCdevkit/VOC2012/', type=str, help="dataset folder")
...
def setup_seed(seed):
    ...
    torch.backends.cudnn.deterministic = False  # Disable for speed (can enable for reproducibility)
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        # Use non_blocking=True to overlap data transfer with computation
        ...
        # Only compute cls_score when logging to avoid blocking GPU every iteration
        cls_score = 0.0
        if (n_iter + 1) % args.log_iters == 0:
            cls_score = evaluate.multilabel_score(cls_label.detach().cpu().numpy()[0], cls_pred.detach().cpu().numpy()[0])

        # Collect metrics asynchronously to avoid blocking - use detach() for all
        avg_meter.add({
            'cls_loss': cls_loss.detach().item(),
            'ptc_loss': ptc_loss.detach().item(),
            'cls_loss_aux': cls_loss_aux.detach().item(),
            'attri_loss': attri_loss.detach().item(),
            'seg_loss': seg_loss.detach().item(),
            'cls_score': cls_score if isinstance(cls_score, (int, float)) else cls_score.item(),
            'cle_loss': cle_loss.detach().item(),
            'une_loss': une_loss.detach().item(),
        })
